...
// SENTIMENT_STYLE — chip palette per customer sentiment.
...
// ConclusionCard — Gemini-generated post-call review fetched lazily per call.
// POSTs /api/transcripts/{id}/conclusion (cached, ?force=1 to regenerate).
// Styled to match the surrounding light-theme modal so labels + body text
// all read against a white card surface.
  ...
  const wrap = (children) => (
    <div style={{
      ...
      <div style={{ fontSize: '0.82rem', fontWeight: 700, color: '#7c3aed', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
        ...
        <button type="button" onClick={() => fetchConclusion(true)} disabled={state.status === 'loading'} title="Regenerate"
          style={{
