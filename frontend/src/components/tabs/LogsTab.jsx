...
export default function LogsTab({ API_URL, authToken, apiFetch }) {
  ...
  // localStorage key for the per-scope "I clicked Clear at this time" marker.
  // Backend replays the last ~20 history items on SSE connect, so without
  // this filter a refresh after Clear would visually undo the clear.
  ...
  useEffect(() => {
    ...
    fetchSseTicket().then(ticket => {
      ...
      es.onmessage = (ev) => {
        ...
        // backend replays the last ~20 history items on every SSE connect.
        const cutoff = clearedAtRef.current;
        if (cutoff > 0) {
          // Try JSON first; if that fails, parse [DD/MM/YYYY HH:MM:SS] prefix.
          let tsMs = NaN;
          try {
            const j = JSON.parse(ev.data);
            if (j && j.ts) tsMs = new Date(j.ts).getTime();
          } catch (_) {
            const m = /\[(\d{2})\/(\d{2})\/(\d{4})\s+(\d{2}):(\d{2}):(\d{2})\]/.exec(ev.data);
            if (m) {
              tsMs = new Date(`${m[3]}-${m[2]}-${m[1]}T${m[4]}:${m[5]}:${m[6]}`).getTime();
            }
          }
  ...
  const handleClear = async () => {
    ...
    // backend still replays history on connect.
    ...
    const cid = campaignFilter || 'all';
    try {
      // Best-effort backend wipe of the Redis history list.
      await apiFetch(`${API_URL}/campaign-events?campaign_id=${cid}`, { method: 'DELETE' });
    } catch (_) { /* network error; client-side filter still applies */ }
  };
