...
// INSERT IGNORE silently drops the row when provider_msg_id collides (duplicate
// delivery from WaSender's retry logic) so the unique index on that column
// is the authoritative dedup guard — the application-level check handles the
// common case and avoids unnecessary DB round-trips.
func (d *DB) SaveWAMessage(conversationID int64, direction, text, msgType, providerMsgID string) (int64, error) {
	// Store empty string as NULL so the unique index allows multiple outbound
	// rows (AI replies, manual sends) that have no provider-assigned ID.
	var providerArg interface{}
	if providerMsgID != "" {
		providerArg = providerMsgID
	}
	res, err := d.pool.Exec(`
		INSERT IGNORE INTO whatsapp_messages (conversation_id, direction, message_text, message_type, provider_msg_id)
		VALUES (?,?,?,?,?)`, conversationID, direction, text, msgType, providerArg)
	if err != nil {
		return 0, err
	}
