...
func TestEnsureCallReviewColumns(t *testing.T) {
	t.Run("applies columns backfills and index", func(t *testing.T) {
		exec := &migrationExecutor{results: make([]migrationExecResult, 5)}

		require.NoError(t, ensureCallReviewColumns(exec))
		require.Len(t, exec.queries, 5)
		require.Contains(t, exec.queries[0], "ADD COLUMN call_outcome")
		require.Contains(t, exec.queries[1], "SET call_outcome='appointment_booked'")
		require.Contains(t, exec.queries[2], "ADD COLUMN lead_id")
		require.Contains(t, exec.queries[3], "SET cr.lead_id = ct.lead_id")
		require.Contains(t, exec.queries[4], "ADD INDEX idx_call_reviews_lead_id")
	})

	t.Run("accepts an already migrated schema", func(t *testing.T) {
		exec := &migrationExecutor{results: []migrationExecResult{
			{err: &mysql.MySQLError{Number: 1060, Message: "Duplicate column name 'call_outcome'"}},
			{},
