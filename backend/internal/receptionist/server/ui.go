...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .past { border-top:1px solid var(--border); padding:12px 16px;
  ...
  .rec { background:var(--panel); border-radius:10px; padding:10px 12px;
         margin-bottom:8px; }
  ...
  .rec-confirm .no  { background:#e5e7eb; color:var(--ink); }
  ...
  .rec-line.user { color:#1d4ed8; }
  .rec-line.assistant { color:var(--ink); }
...
<div class="app">
  <header>
    ...
    <div class="controls">
      <label class="pill" style="background:var(--panel);">Mode</label>
      <select id="modeSel">
        <option value="chat">Chat</option>
        <option value="call">Call</option>
      </select>
      <label class="pill" style="background:var(--panel);">Voice</label>
