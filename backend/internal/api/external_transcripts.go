...
//
// Auth: standard JWT — partner logs in once via POST /api/login, caches the
// token, re-logs when it 401s. Org-scoped through ac.OrgID so a partner can
// only see their own org's data.
//
// Query params:
//
//	?campaign_id=<id>  — restrict to one campaign. Omit to get all campaigns
//	                     in the org. Useful for pagination by campaign when
//	                     the org grows past what a single response can carry.
//
// Conclusion field is null when no call_reviews row exists (e.g. very short
// calls where analysis was skipped). The shape is otherwise consistent so
// the partner can pre-allocate / map it without conditional branches.
//
// Response shape:
//
//	[
...
func (s *Server) getExternalTranscripts(w http.ResponseWriter, r *http.Request) {
	...
	// Optional ?campaign_id=N filter — partner teams iterate one campaign at
	// a time when the org is large. Invalid values are ignored rather than
	...
	for _, c := range campaigns {
		...
		if err != nil {
			...
			// Don't bail on the whole response — show the campaign with an
			// empty leads array so partner code can still see it exists.
			leads = nil
		}
		...
		for _, l := range leads {
			...
			// would otherwise show duplicate calls under every campaign the
			// lead appears in.
			...
			for _, t := range transcripts {
				...
				if review, _ := s.db.GetCallReviewByTranscript(t.ID); review != nil {
					conclusion = map[string]any{
						...
						"customer_sentiment":            review.CustomerSentiment,
				...
				calls = append(calls, map[string]any{
					...
					"transcript":    turns,
					"conclusion":    conclusion, // null when no review row
				})
			}
